...
2. **[build-agent-example/](build-agent-example/)** — 12 个渐进式教学示例（step01 → step12），从单次对话到 Tool Use + Skills + Todolist + Subagent + Agent Team + MCP + Hooks + **Goal**。每个示例都配有同名讲解文档。
...
如果你已经理解本仓库 step01 → step12 的核心原理，可以继续看一个更完整的产品化形态：
...
agent/
├── loop.py                 主循环 + 全部组件装配
├── runner.py               单轮 messages.create + tool_use 循环 + 安全工具并发 + hooks
├── memory.py               三层记忆存储
...
├── goals/                  目标驱动系统
│   ├── store.py            GoalStore：目标树 + 依赖 + 持久化
│   └── evaluator.py        GoalEvaluator：完成判定
├── hooks/                  生命周期钩子
│   ├── base.py             Hook 基类
│   ├── registry.py         HookRegistry
│   └── builtins.py         LoggingHook / RateLimitHook / ToolAuditHook
...
主 Agent 在关键生命周期节点暴露了 hook 事件，支持以插件方式接入监控、限流、审计等横切逻辑，无需修改 `AgentRunner` 核心代码。

已支持事件：

- `on_user_input` / `on_assistant_reply`：用户输入 / 助手回复；
- `before_turn` / `after_turn`：单轮 LLM 调用前后；
- `before_tool_call` / `after_tool_call`：单个工具执行前后；
- `before_compact` / `after_compact`：历史压缩前后。

用法：

```python
from agent import Hook, HookRegistry

class MyHook(Hook):
    name = "my_hook"
    def after_turn(self, ctx):
        print(f"turn finished: {ctx['reply']}")

loop = AgentLoop()
loop.hooks.register(MyHook())
loop.run()
```

内置 hooks：

- `LoggingHook`：打印每轮耗时与 token；
- `RateLimitHook`：限制窗口期最大 LLM 调用次数；
- `ToolAuditHook`：记录 `write_file` / `run_command` 等写操作到 `.hooks_audit.jsonl`。

主 runner 使用完整 hooks；子代理和固定队友只使用 `safe_hooks`（默认仅 `LoggingHook`），避免审计类 hook 泄露主 Agent 上下文。

### Goal 目标驱动

当用户输入 `/goal 目标描述` 时，主 Agent 进入目标模式：

1. 把用户目标作为 root goal 写入 `GoalStore`；
2. 重建 system prompt，注入当前目标树；
3. 在 `max_autonomous_turns` 限制内自主循环调用 `runner.step()`；
4. 每轮检查目标树状态，全部完成或达到上限时退出并汇总。

目标树支持：

- `parent_id`：形成层级结构；
- `depends_on`：依赖全部完成后才能开始；
- `success_criteria`：完成判定标准；
- `result`：执行结果摘要。

目标持久化到 `memory/goals.jsonl`，跨会话可恢复。
...
| 11 | [step11_hooks.py](build-agent-example/code/step11_hooks.py) | [doc](build-agent-example/doc/step11_hooks.md) | Hooks 生命周期 | before_turn / after_tool_call / 限流 / 审计 |
| 12 | [step12_goal.py](build-agent-example/code/step12_goal.py) | [doc](build-agent-example/doc/step12_goal.md) | 目标驱动 | 目标树 + 自主推进 + 完成判定 |
