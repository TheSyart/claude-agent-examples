#!/usr/bin/env python3
"""step13_goal.py — 累积式 Agent：在 step12 Hooks 基础上加入 Goal。"""
...
GOALS_FILE = Path(__file__).parent / ".goals.jsonl"
MAX_AUTONOMOUS_TURNS = int(os.environ.get("MAX_AUTONOMOUS_TURNS", "6"))
...
def execute_main_tool(block) -> str:
    ...
    if name in ("run_command", "web_fetch", "load_skill"):
        ...
        output = "Error:\n" + "\n".join(errors) if errors else "目标树已更新：\n" + GOAL_STORE.render()
...
class Goal:
    ...
    def __init__(self, **kwargs):
        ...
        self.depends_on = list(kwargs.get("depends_on") or [])
...
class GoalStore:
    ...
    def update(self, items: list[dict]) -> list[str]:
        errors = []
        new_goals = {}
        new_order = []
        for item in items:
            try:
                goal = Goal(**item)
                if goal.status not in Goal.VALID_STATUSES:
                    raise ValueError(f"invalid status {goal.status}")
            except Exception as exc:
                errors.append(f"invalid goal {item.get('id', '?')}: {exc}")
                continue
            new_goals[goal.id] = goal
            new_order.append(goal.id)

        for goal in new_goals.values():
            for dep in goal.depends_on:
                if dep not in new_goals:
                    errors.append(f"goal '{goal.id}' depends on unknown '{dep}'")
            if goal.parent_id and goal.parent_id not in new_goals:
                errors.append(f"goal '{goal.id}' has unknown parent '{goal.parent_id}'")
    ...
    def complete(self, goal_id: str, result: str = "") -> None:
        goal = self.goals.get(goal_id)
        if goal:
            goal.status = "completed"
            goal.result = result or goal.result
            self._persist()

    def has_pending_or_in_progress(self) -> bool:
        return any(g.status in {"pending", "in_progress"} for g in self.goals.values())

    def children_of(self, parent_id: str | None) -> list[Goal]:
        return [g for g in self.goals.values() if g.parent_id == parent_id]

    def render_tree(self, parent_id=None, depth=0) -> str:
        children = sorted(
            self.children_of(parent_id),
            key=lambda g: self._order.index(g.id) if g.id in self._order else 0,
        )
        if not children:
            return ""
        ...
        for child in children:
            prefix = "  " * depth
            dep = f" [depends: {', '.join(child.depends_on)}]" if child.depends_on else ""
            lines.append(f"{prefix}{icon.get(child.status, '?')} [{child.status}] {child.content}{dep}")
            lines.append(self.render_tree(child.id, depth + 1))
        return "\n".join(line for line in lines if line)

    def render(self) -> str:
        return self.render_tree() or "(暂无目标)"
    ...
    def load_latest(self) -> None:
        ...
        try:
            ...
            return
        self._order = snapshot.get("order", [])
        self.goals = {g["id"]: Goal(**g) for g in snapshot.get("goals", [])}
...
使用 update_goals 维护目标树：
- 把大目标拆成子目标，可设 parent_id 和 depends_on。
- 开始执行某个目标时设为 in_progress，完成后设为 completed，并记录 result。
- 自主目标模式下，全部完成后请在最终回复中包含 DONE: 标记。"""
...
TOOLS = [
    ...
    {
        "name": "update_goals",
        "description": "全量更新目标树，用于拆解复杂目标、标记完成状态。",
        "input_schema": {
            "type": "object",
            "properties": {
                "goals": {
...
print("Step 13: 累积式 Agent（step12 + Goal）")
print("已连接 MCP Server:", list(MCP_CLIENTS.keys()) or "无")
print("输入 /goal 目标描述 可进入自主目标模式")


def run_agent_cycle(history: list[dict]) -> str:
    """运行一次完整 Agent 循环，直到模型给出非 tool_use 回复。"""
    global TODOS
    while True:
        turn_ctx = {
            "history": history,
...
def run_goal_mode(goal_text: str, history: list[dict]) -> str:
    """自主目标模式：多轮调用同一个累积式 Agent，直到目标完成或达到上限。"""
    GOAL_STORE.update([{"id": "root", "content": goal_text, "status": "in_progress"}])
    user_message = {"role": "user", "content": goal_text}
    history.append(user_message)
    MEMORY.append_history(user_message)

    final_reply = ""
    for turn in range(MAX_AUTONOMOUS_TURNS):
        print(f"\n[目标模式 第 {turn + 1}/{MAX_AUTONOMOUS_TURNS} 轮]")
        ...
        if turn < MAX_AUTONOMOUS_TURNS - 1:
            continue_message = {"role": "user", "content": "继续推进目标，直到全部完成。完成后在回复中包含 DONE:。"}
            history.append(continue_message)
            MEMORY.append_history(continue_message)
    else:
        final_reply = f"（达到最大自主轮数 {MAX_AUTONOMOUS_TURNS}，目标尚未全部完成）\n{final_reply}"
    return final_reply
...
while True:
    ...
    if command == "/mcp":
        ...
        continue
    if command == "/goals":
        print(GOAL_STORE.render())
        print()
        continue
    if command.startswith("/goal"):
        goal_text = command[len("/goal"):].strip()
        if not goal_text:
            print("请提供目标，例如：/goal 统计当前目录下所有 .py 文件行数\n")
            continue
        run_goal_mode(goal_text, history)
        continue

    user_message = {"role": "user", "content": user_input}
    history.append(user_message)
    MEMORY.append_history(user_message)
    run_agent_cycle(history)
